...
- **Données locales** — `data/LissafiDatabase.kt` : `SQLiteOpenHelper` **brut (SQL brut, pas Room** — le plan doc dit Room mais le code réel est du raw SQL). Contient aussi les extensions `Cursor.toX()` et les méthodes de synchro (getUnsyncedSales, finalizeSalePush…). `DATABASE_VERSION = 7` (v2 = colonne `user_id` ; v3 = PK composite `(barcode, user_id)` + soft delete sur `products` ; v4 = suppression du PIN admin ; v5 = colonne `synced` locale sur `debt_transactions` ; v6 = colonne `shop_id` sur les 5 tables métier — offre Grand boutique, boutique partagée ; v7 = table `cash_closures` — clôture de caisse « Z »).
