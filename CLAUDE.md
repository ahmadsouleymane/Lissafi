...
- **Portail partenaire auto-serveur** (routes publiques `src/app/partenaire/*`, hors groupe `(admin)`) : auto-inscription (`partnerSignUp` crée un user Supabase Auth via `service_role` + ligne `partners.auth_uid`), espace `/partenaire/espace` (lien perso `?p=CODE` + QR, stats, demande de retrait). Cookie httpOnly **distinct** `lissafi_partner_token`, session via `getPartnerSession()`. Les tables partenaires restent **admin-only** (RLS `is_admin()`) : le portail lit/écrit via `service_role` filtré par l'id du cookie, jamais depuis le navigateur. Beacon de visite public : `src/app/api/visits/route.ts` (POST `{code}`, CORS restreint à `NEXT_PUBLIC_LANDING_URL`). Admin : `/partenaires/retraits` valide les retraits (`approvePayout` solde les plus anciennes commissions dues). Design : `docs/superpowers/specs/2026-08-13-portail-partenaire-design.md`.
