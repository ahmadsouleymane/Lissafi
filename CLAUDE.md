...
- **Données locales** — `data/LissafiDatabase.kt` : `SQLiteOpenHelper` **brut (SQL brut, pas Room** — le plan doc dit Room mais le code réel est du raw SQL). Contient aussi les extensions `Cursor.toX()` et les méthodes de synchro (getUnsyncedSales, finalizeSalePush…). `DATABASE_VERSION = 5` (v2 = colonne `user_id` ; v3 = PK composite `(barcode, user_id)` + soft delete sur `products` ; v4 = suppression du PIN admin ; v5 = colonne `synced` locale sur `debt_transactions`).
