...
/** Le portail est opérationnel si les 2 clés sont renseignées. */
export function isConfigured(): boolean {
  return Boolean(getAnonKey() && getServiceKey());
}

/** Client "auth" — connexion partenaire (GoTrue) avec la clé anon. */
 ...
 * Bypasse la RLS : réservé au code serveur du portail, filtré par l'id partenaire.
 */
