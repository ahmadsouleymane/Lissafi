...
/** Demandes de retrait des partenaires (récentes d'abord), avec identité jointe. */
export async function getPayoutRequests(limit = 200): Promise<PayoutRequest[]> {
  const { data, error } = await supabaseAdmin()
    ...
    .limit(limit);
  if (error) {
    console.error("[admin] getPayoutRequests:", error);
    return [];
  }
  const payouts = (data ?? []) as PartnerPayout[];
  if (payouts.length === 0) return [];

  const ids = Array.from(new Set(payouts.map((p) => p.partner_id)));
  ...
  return payouts.map((p) => {
    const info = map.get(p.partner_id);
    return {
      ...p,
      partner_name: (info?.name as string) ?? "—",
      partner_phone: (info?.phone as string) ?? "",
      partner_code: (info?.code as string) ?? "",
    };
  });
}
...
export async function getPartnerVisits(partnerId: string, limit = 30): Promise<PartnerVisit[]> {
  const { data, error } = await supabaseAdmin()
    ...
    .limit(limit);
  if (error) {
    console.error("[partner] getPartnerVisits:", error);
    return [];
  }
...
/**
 * Tableau de bord complet de l'espace partenaire (portail auto-serveur).
 * Toutes les lectures via service_role, filtrées par l'id du partenaire du cookie.
 */
export async function getPartnerDashboard(partnerId: string): Promise<PartnerDashboard | null> {
  ...
  if (partnerError || !partner) {
    if (partnerError) console.error("[partner] getPartnerDashboard:", partnerError);
    return null;
  }
